    ...
    <a-space direction="vertical" size="large" fill>
      ...
      <div class="carousel-container">
        <a-carousel
          ...
          show-arrow="hover"
        >
          <a-carousel-item v-for="(id, index) in newIds" :key="index">
            <SearchVideo
              :key="id"
              :id="id"
              :height="400"
      ...
      <div class="video-container">
        <div v-for="id in hotFreeIds" :key="id" class="video-item">
          <SearchVideo
            :key="id"
            :id="id"
            :height="200"
      ...
      <p>热门收费课程</p>
      <div v-for="id in hotPaidIds" :key="id">
        <SearchVideo
          :key="id"
          :id="id"
          :height="200"
...
import { BaseResponse_HomePageDisplayDto_ } from "../../../generated/models/BaseResponse_HomePageDisplayDto_";
...
const hotPaidIds = ref<number[]>([]);
const hotFreeIds = ref<number[]>([]);
/**
 * 页面加载时，请求数据
 */
onMounted(() => {
  ...
  Service.displayUsingGet().then((res: BaseResponse_HomePageDisplayDto_) => {
    if (res.code === 0) {
      newIds.value = res.data?.newIds ?? [];
      hotPaidIds.value = res.data?.hotPaidIds ?? [];
      hotFreeIds.value = res.data?.hotFreeIds ?? [];
    } else {
      message.error(res.message as string);
    }
  });
...
#TechieNestView .menu {
  //margin-bottom: 30px; //border: 2px solid green; /* 设置边框为2像素实线绿色 */ //border-radius: 50px; /* 较大的值确保两侧为半圆形状 */ //font-size: 16px; /* 字体大小可以根据实际需求调整 */
}

#TechieNestView .carousel-container {
  display: flex;
